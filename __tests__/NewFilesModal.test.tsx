...
jest.mock("../src/client/components/NewFilesCourseCollapsable", () => ({
  NewFilesCourseCollapsable: ({ name, files }: any) => (
    <div data-testid={`course-${name}`}>
      {files.map((f: any, idx: number) => <span key={idx}>{f.filename}</span>)}
    </div>
  )
}))
...
describe("NewFilesModal", () => {
  it("renders with correct count and passes files down", () => {
    ...
    const mockFiles = {
      "Course 1": [{ filename: "file1.pdf", filepath: "/1", timecreated: 0, absolutePath: "/1", filesize: 10, updated: false }, { filename: "file2.pdf", filepath: "/2", timecreated: 0, absolutePath: "/2", filesize: 20, updated: false }],
      "Course 2": [{ filename: "file3.pdf", filepath: "/3", timecreated: 0, absolutePath: "/3", filesize: 30, updated: false }]
    } as any
